...
这套系统服务的是中文界面，下面三条不可协商。任何一次「让它更紧凑一点」的
改动，都要先过这三条。
...
和 `--color-surface-alt`（#f6fbfb）**两个底**上都过 4.5:1 —— 一个在白底上达标的
颜色，压到 surface-alt 上可能掉到 4.5 以下。而且过了比例不等于可读：上面两条
描述的正是「过了 AA 却读不了」的情况。

完整规范见 [docs/index.html](docs/index.html)，含 14 组对比度实测值。
...
**中文不加载网络字体。** 一页中文标题只有几十个字，而一套中文网络字体按
unicode-range 分片后实测仍需下载约 250KB —— 标题的性格来自拉丁展示体，
中文黑体之间的差异抵不上这个体积。
